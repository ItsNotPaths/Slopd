...
filetree_scroll_apply :: proc(ft: ^FileTree, rows: int, center: bool, last_input_at: f64 = 0) {
    ui.list_scroll_apply(&ft.scroll, &ft.scroll_detached, ft.selected, rows, len(ft.entries), center, last_input_at)
}
    ...
    if clay.UI(clay.ID("ft_pane"))(ui.clay_pane_box(area)) {
        if clay.UI(clay.ID("ft_head"))(
            ...
            if wsfind_shown(a) {
                ...
                clay.Text(ft.dir, ui.clay_text_config(th.muted, lh))
            }
        }
        ...
        if wsfind_shown(a) {
            wsfind_declare_body(u, &a.wsfind, body, row_h, lh, cw, now)
        } else {
            filetree_declare_rows(u, ft, edit.ring_dirty(&a.editor), body, top, off, row_h, lh, cw)
        }
...
filetree_body_rect :: proc(area: gfx.Rect, row_h: i32) -> gfx.Rect {
    return gfx.Rect{area.x, area.y + row_h, area.w, max(0, area.h - row_h)}
}
...
@(private = "file")
filetree_declare_rows :: proc(u: ui.UI_Ctx, ft: ^FileTree, dirty: ui.Path_Dirty, body: gfx.Rect, top: int, off, row_h, lh: i32, cw: f32) {
    th := u.theme
    first := clamp(top, 0, max(0, len(ft.entries)))
    visible := max(0, min(len(ft.entries) - first, ui.list_visible_rows(body.h, off, row_h)))
    ...
    if clay.UI(clay.ID("ft_body"))(
        ...
        for k in 0 ..< visible {
            ...
            ringed := ui.path_is_dirty(dirty, e.path)
            ...
            } else if ui.hover_shown(u) && i == ft.hover {
                bg = ui.clay_rgb(ui.hover_bg(th)) // last, so it never masks a mark
            }
            ...
            if clay.UI(clay.ID("ft_row", u32(i)))(
                ...
                if clay.UI(clay.ID("ft_pre", u32(i)))(
                    ...
                    clay.Text(prefix, ui.clay_text_config(pcol, lh))
                }
                clay.Text(e.display, ui.clay_text_config(e.is_dir ? th.code_return_type : th.fg, lh))
            }
        }
    }
}
...
filetree_frame :: proc(t: ^gfx.Draw, a: ^App, pane: gfx.Rect, now: f64) {
    ...
    if wsfind_shown(a) {
        ...
        head := gfx.Rect{area.x + i32(cw), area.y, max(0, area.w - i32(cw)), row_h}
        body := filetree_body_rect(area, row_h)
        ...
        top0, off0 := ui.smooth_scroll(&a.tree.scroll_anim, a.tree.scroll, now, row_h)
        hit := filetree_hit(&a.tree, top0, ui.list_visible_rows(area.h - row_h, off0, row_h))
        a.tree.hover = hit // against the same (last) frame the click is
        filetree_click(a, hit)
        filetree_rclick(a, hit)
        filetree_scroll_apply(&a.tree, rows, u.scroll_mode == .Middle, ui.pane_input_at(u))
