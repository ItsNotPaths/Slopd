...
wsfind_open :: proc(a: ^App) {
    ...
    ws.off = 0
    search.wsfind_scan(ws, a.project_root, exclude_dirs(a))
    wsfind_build(a)
}
...
wsfind_sync :: proc(a: ^App) {
    ...
    if ws.query.version != ws.ver || (!search.wsfind_typed(ws) && wsfind_ring_stale(a)) {
        wsfind_build(a)
    }
}
...
wsfind_activate :: proc(a: ^App) {
    row, ok := search.wsfind_selected(&a.wsfind)
    if !ok {
        return
    }
...
wsfind_build :: proc(a: ^App) {
    ...
    if q == "" {
        ...
        for want_dirty in ([?]bool{true, false}) {
            for &b in a.editor.buffers {
                if b.dirty == want_dirty && edit.buffer_on_disk(&b) {
                    append(&ws.rows, search.WS_Row{strings.clone(b.path), b.dirty})
                }
            }
        }
    } else {
...
wsfind_rank :: proc(a: ^App, q: string) {
    ws := &a.wsfind
    hits := make([dynamic]search.WS_Hit, 0, 64, context.temp_allocator)
    for p in ws.files {
        if s, ok := search.wsfind_score(search.wsfind_rel(ws.root, p), q); ok {
            append(&hits, search.WS_Hit{p, s})
        }
    }
    slice.sort_by(hits[:], wsfind_better)
    for h in hits[:min(len(hits), search.WS_ROWS_MAX)] {
        append(&ws.rows, search.WS_Row{strings.clone(h.path), edit.ring_contains(&a.editor, h.path)})
    }
}
...
@(private = "file")
wsfind_better :: proc(x, y: search.WS_Hit) -> bool {
    if x.score != y.score {
