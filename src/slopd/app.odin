...
App :: struct {
    ...
    ctrl_held:   bool,
    shift_held:  bool,
    terminals:   [dynamic]^pty.Terminal,
    term_active: int,
    ...
    // a fuzzy match over the project's files once you do. Covers the listing (src/search).
    wsfind:      search.WS_Find,

    // The one popup in the program (src/ui). Owned here rather than by a pane: it
    ...
    // split at each use by exclude_dirs (src/search), where a pattern's meaning is written.
    exclude: string,
...
active_doc :: proc(a: ^App) -> ^txt.Doc {
    ...
    return &edit.editor_current(&a.editor).doc
}
...
view_refresh :: proc(a: ^App) {
    ...
    case .Text:
        if len(a.editor.buffers) > 0 {
            ...
            was := b.conflict
            edit.buffer_reload_if_changed(b, a.conflict_prompt)
...
app_init :: proc(a: ^App) {
    ...
    cl_init(&a.cl)
    search.wsfind_init(&a.wsfind) // its file list is scanned when it opens
...
font_set_px :: proc(a: ^App, px: f32) {
    next := clamp(px, FONT_PX_MIN, FONT_PX_MAX)
    if next == a.font_px {
        return
    }
    a.font_px = next
...
app_destroy :: proc(a: ^App) {
    ...
    search.wsfind_destroy(&a.wsfind)
    find_destroy(&a.find)
    ctxmenu_destroy(a)
