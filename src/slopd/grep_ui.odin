...
grep_scroll_apply :: proc(g: ^search.GrepPane, anchor, rows, total: int, center: bool, last_input_at: f64 = 0) {
...
grep_hit :: proc(rows: []search.GrepRow, first, max_rows: int) -> int {
...
grep_declare :: proc(u: ui.UI_Ctx, g: ^search.GrepPane, face: gfx.Face, pane: gfx.Rect, rows: []search.GrepRow) {
    ...
    if clay.UI(clay.ID("gp_pane"))(ui.clay_pane_box(area)) {
        if clay.UI(clay.ID("gp_head"))(
            ...
            clay.Text(search.grep_head(g), ui.clay_text_config(focus_fg(u, .Aux), lh))
        }
...
grep_frame :: proc(t: ^gfx.Draw, a: ^App, pane: gfx.Rect) {
    ...
    rows := search.grep_rows(g, a.project_root, context.temp_allocator)
