...
Preview_Save :: struct {
    ...
    grep:             search.GrepPane, // for `:grep`, the whole pane it moved aside
...
@(private = "file")
cl_preview_build :: proc(a: ^App, b: ^edit.Buffer) {
...
@(private = "file")
preview_jump :: proc(a: ^App, b: ^edit.Buffer, args: string) {
    if b == nil {
        return
    }
...
@(private = "file")
preview_find :: proc(a: ^App, b: ^edit.Buffer, args: string) {
...
preview_search :: proc(a: ^App, kind: Preview_Kind, query, replace: string, replacing: bool) {
    ...
    a.grep = {}
    search.grep_set(&a.grep, query, nil, replace, replacing) // the query now; the worker fills the hits in
...
@(private = "file")
preview_begin :: proc(a: ^App, b: ^edit.Buffer, kind: Preview_Kind) {
...
preview_end :: proc(a: ^App) {
    ...
    search.grep_destroy(&a.cl_preview.saved.grep)
    a.cl_preview.saved = {}
