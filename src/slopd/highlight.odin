...
@(private = "file")
highlighter_tree :: proc(h: ^Highlighter, b: ^edit.Buffer, lang: ts.Language) -> (tree: ts.Tree, current: bool) {
    hl_publish(h)
    hl_request(h, b, lang)
...
// from nothing.
@(private = "file")
hl_request :: proc(h: ^Highlighter, b: ^edit.Buffer, lang: ts.Language) {
...
hl_settle :: proc(a: ^App, b: ^edit.Buffer) {
...
highlight_visible :: proc(a: ^App, b: ^edit.Buffer, first_line, count: int) -> []Row_Colors {
    g, ok := highlighter_grammar(a, b.path)
    if !ok || g.query == nil {
        return nil
    }
...
highlight_dump_captures :: proc(a: ^App, b: ^edit.Buffer, first_line, count: int) {
    g, ok := highlighter_grammar(a, b.path)
    if !ok || g.query == nil {
...
highlight_fold_range :: proc(a: ^App, b: ^edit.Buffer, line: int) -> (start, end: int, ok: bool) {
...
ts_in_string_or_comment :: proc(a: ^App, b: ^edit.Buffer, line, col: int) -> bool {
    g, found := highlighter_grammar(a, b.path)
    if !found {
        return false
    }
...
// (path, line).
ts_filter_definitions :: proc(a: ^App, query: string, hits: []search.GrepHit) -> []search.GrepHit {
    h := &a.hl
    out := make([dynamic]search.GrepHit, 0, len(hits), context.temp_allocator)
    ...
    for hit in hits {
        ...
        if col, ok := ts_def_col_on_line(cur_tree, cur_src, cur_starts, hit.line - 1, query); ok {
            append(&out, search.GrepHit{path = hit.path, line = hit.line, col = col, text = hit.text})
        }
    }
...
@(private = "file")
dedup_hits :: proc(hits: []search.GrepHit) -> []search.GrepHit {
    out := make([dynamic]search.GrepHit, 0, len(hits), context.temp_allocator)
