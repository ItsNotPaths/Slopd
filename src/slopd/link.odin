...
// Only the dispatch and the URL/wiki/colour handling live here; the scan is src/search and the
...
link_jump_definition :: proc(a: ^App, ident: string) {
    ...
    case:
        search.grep_set(&a.grep, ident, defs) // several: list them to pick from
        set_aux(a, .Grep)
    }
}
...
// The shallowest match. Skips dotted directories and the configured exclusions (src/search),
    ...
    for i := 0; i < len(queue) && i < FIND_DIR_BUDGET; i += 1 {
        ...
        for fi in os.read_directory_iterator(&it) {
            if fi.type == .Directory {
                ...
                if !dotted && !search.exclude_hit(exclude, fi.name) {
