...
grep_open_hit :: proc(a: ^App, h: search.GrepHit) {
...
// skips `vendor`.
grep_project :: proc(a: ^App, query: string, word := false, fixed := false) -> []search.GrepHit {
    return search.grep_run(a.project_root, query, exclude_dirs(a), word, fixed)
}
