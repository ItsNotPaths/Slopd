...
char_callback :: proc "c" (window: glfw.WindowHandle, codepoint: rune) {
    ...
    if tf := term_focused(a); tf != nil {
        pty.terminal_input_rune(tf, codepoint)
        return
    }
    ...
    case .None:
        return
    case .Buffer:
        b := edit.editor_current(&a.editor) // the one editable that closes a bracket
        if !buffer_autopair(b, codepoint) {
            edit.buffer_insert_rune(b, codepoint)
        }
    case .Config_Search:
...
handle_key :: proc(a: ^App, key, action, mods: i32) {
    ...
    if ctxmenu_shown(a) && !ui.key_is_modifier(key) {
        switch key {
    ...
    if tf := term_focused(a); tf != nil {
        pty.terminal_input_key(tf, key, mods)
    }
}
