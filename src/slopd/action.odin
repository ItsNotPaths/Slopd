...
grep_target :: proc(a: ^App) -> ^search.GrepPane {
...
action_run :: proc(a: ^App, act: Action, n: int, extend, all: bool) -> (handled: bool) {
    ...
    case .Split_Shrink:
        a.split = clamp(a.split - 0.02, SPLIT_MIN, SPLIT_MAX)
    case .Split_Grow:
        a.split = clamp(a.split + 0.02, SPLIT_MIN, SPLIT_MAX)
    case .Escape:
        escape_run(a, all)
    ...
    case .Term_Sel_Up, .Term_Sel_Down:
        ...
        pty.terminal_sel_move(ts, act == .Term_Sel_Up ? -1 : 1, extend)
    ...
    case .Jump_Up, .Jump_Down:
        ...
        if b := buffer_target(a); b != nil {
            edit.buffer_motion(b, up ? .Up : .Down, extend, all, a.jump_lines)
            return true
        }
...
nav_run :: proc(a: ^App, n: Nav, extend, all: bool) -> bool {
    ...
    case .Buffer:
        ...
        switch n {
        case .Up:
            edit.buffer_motion(b, .Up, extend, all)
        case .Down:
            edit.buffer_motion(b, .Down, extend, all)
        case .Left, .Right:
...
activate_run :: proc(a: ^App, extend: bool) -> bool {
    ...
    case .Buffer:
        buffer_enter(a, edit.editor_current(&a.editor)) // auto-indent / brace expansion
        return true
    case .None:
    }
...
delete_run :: proc(a: ^App, act: Action) -> bool {
    ...
    if b := buffer_target(a); b != nil {
        switch {
        case back && word: edit.buffer_delete_word_back(b)
        case back:         edit.buffer_backspace(b)
        case word:         edit.buffer_delete_word_forward(b)
        case:              edit.buffer_delete(b)
        }
        return true
    }
    switch {
...
escape_run :: proc(a: ^App, move_all_pending: bool) {
    ...
    case ts != nil && (ts.sel_active || ts.msel_on):
        ...
        pty.terminal_sel_reset(ts)
    case tf != nil:
        pty.terminal_input_key(tf, glfw.KEY_ESCAPE, 0) // vim and its kin must see it
