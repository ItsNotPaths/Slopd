...
buffer_autopair :: proc(b: ^edit.Buffer, r: rune) -> bool {
...
buffer_tab :: proc(b: ^edit.Buffer, indent: txt.Indent) {
    d := &b.doc
...
buffer_indent_block :: proc(b: ^edit.Buffer, indent: txt.Indent, out: bool) -> bool {
    d := &b.doc
...
dedent_width :: proc(src: []u8, indent: txt.Indent) -> int {
    if len(src) > 0 && src[0] == '\t' {
        return 1
    }
    n := 0
...
indent_text :: proc(indent: txt.Indent) -> string {
