...
Grep_Answer :: struct {
    query:     string,
    hits:      []search.GrepHit, // context blocks already filled
    want:      Grep_Want,
    seq:       u64,
    replace:   string,
    replacing: bool,
...
grep_apply :: proc(a: ^App, ans: Grep_Answer) {
    ...
    case .Preview:
        if preview_owns_grep(a) { // still up: an Esc took the pane back otherwise
            search.grep_set(&a.grep, ans.query, ans.hits, ans.replace, ans.replacing)
        }
    case .List:
        search.grep_set(&a.grep, ans.query, ans.hits, ans.replace, ans.replacing)
        set_aux(a, .Grep)
...
grep_worker_proc :: proc(th: ^thread.Thread) {
    ...
    for {
        ...
        alloc := grep_job_allocator()
        hits := search.grep_run(job.root, job.query, job.exclude, false, job.replacing)
        ans := Grep_Answer {
            query     = strings.clone(job.query, alloc),
            hits      = search.grep_hits_clone(hits, alloc), // the disk reads, off the main thread too
            want      = job.want,
            seq       = job.seq,
...
grep_answer_destroy :: proc(ans: ^Grep_Answer) {
    ...
    search.grep_hits_destroy(ans.hits, alloc)
    ans^ = {}
}
