...
// grammars/ is a data folder (src/paths), created on demand. The `languages` registry itself
...
// A data folder, so src/paths decides where it sits. Returned existing or not: the first
// install creates it. The caller owns the result.
grammars_dir :: proc(allocator := context.allocator) -> string {
    return paths.data_asset("grammars", allocator)
}
...
grammar_install :: proc(dir: string, g: Grammar) -> (ok: bool, msg: string) {
    ...
    build := filepath.join({tmp, fmt.tprintf("%s-grammar-%s", paths.app_name, g.name)}, context.temp_allocator) or_else ""
...
// --- CLI (`--health [lang]`, `--grammar <action> <lang>`) ---
...
grammar_cli :: proc(args: []string) -> (handled: bool) {
    for i := 0; i < len(args); i += 1 {
        ...
        case "--grammar":
            if i + 2 >= len(args) {
                fmt.eprintfln("usage: %s --grammar install|uninstall|update <lang>", paths.app_name)
                return true
            }
