...
terminal_sync :: proc(term: ^Terminal, th: ^gfx.Theme, cols, rows: int) {
...
terminal_view :: proc(term: ^Terminal, area: gfx.Rect, row_h: i32, cw: f32, cols, rows: int) -> Terminal_View {
...
// a hit.
terminal_hit :: proc(u: ui.UI_Ctx, term: ^Terminal, v: Terminal_View) -> Terminal_Hit {
...
terminal_click :: proc(u: ui.UI_Ctx, term: ^Terminal, active: int, hit: Terminal_Hit) {
    if !hit.ok || term == nil {
        return
    }
    ...
    if term.mouse_on && !u.mouse.click_shift && hit.live {
        terminal_mouse_tap(term, hit.line - term.sb_total, hit.col, u.mouse.click_ctrl)
        return
    }
...
terminal_drag_line :: proc(u: ui.UI_Ctx, term: ^Terminal, v: Terminal_View, line: int, now: f64) -> int {
    past, dir := 0, 0
    switch {
    ...
    if ui.drag_tick(u, now) {
        ...
        u.drag.over = clamp(u.drag.over + dir * ui.drag_scroll_step(past, int(v.row_h)), floor, terminal_bottom(term))
...
terminal_drag :: proc(u: ui.UI_Ctx, term: ^Terminal, v: Terminal_View, active: int, now: f64) {
    ...
    terminal_msel_head(term, txt.Pos{line, bound.col})
}
...
terminal_declare :: proc(u: ui.UI_Ctx, a: ^App, f: ^gfx.Font, term: ^Terminal, v: Terminal_View, now: f64 = 0) {
    area := v.area
