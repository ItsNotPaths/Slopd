...
terminal_hit :: proc(a: ^App, term: ^Terminal, v: Terminal_View) -> Terminal_Hit {
    ...
    // Not gated on a.mouse.stood_down: that suppresses hover, and this pane paints none — the
...
terminal_paint_grid :: proc(t: ^Text, r, clip: Rect, win_w, win_h: i32, a: ^App, user: rawptr) {
    ...
    fill(t, r, th.bg) // the whole grid's default background, in one quad
...
terminal_frame :: proc(t: ^Text, a: ^App, pane: Rect, now: f64) {
    ...
    terminal_drag(a, term, v, glfw.GetTime()) // extend a capture the press already made
