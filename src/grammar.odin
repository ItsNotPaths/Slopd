...
// Loads the language registry from the `languages` file beside the binary (generated by
// tools/gen-languages.py from Helix). Tab-separated, '-' for empty fields:
//   name <TAB> repo <TAB> rev <TAB> subpath <TAB> ext,ext,...
// Empty slice if the file is missing (run download-deps.sh). The result is owned by
// context.allocator — free it with grammars_destroy under that same context.
