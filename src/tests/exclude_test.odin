...
test_exclude_split :: proc(t: ^testing.T) {
    pats := app.exclude_split(".git, vendor, node_modules")
...
test_exclude_hit :: proc(t: ^testing.T) {
    pats := app.exclude_split("vendor, node_modules, *.bundle")
    testing.expect(t, app.exclude_hit(pats, "vendor"))
    testing.expect(t, app.exclude_hit(pats, "node_modules"))
    testing.expect(t, app.exclude_hit(pats, "assets.bundle"))
    testing.expect(t, !app.exclude_hit(pats, "vendored"), "a pattern matches the WHOLE name")
    testing.expect(t, !app.exclude_hit(pats, "src"))
    testing.expect(t, !app.exclude_hit(nil, "vendor"), "an empty list excludes nothing")

    // A pattern that will not compile matches nothing: a typo must not empty the jump list.
    testing.expect(t, !app.exclude_hit(app.exclude_split("[bad"), "anything"))
}
...
test_grep_argv :: proc(t: ^testing.T) {
    argv := app.grep_argv("/w", "needle", app.exclude_split(".git, vendor"), false, false)
    ...
    word := app.grep_argv("/w", "-x", nil, true, true)
