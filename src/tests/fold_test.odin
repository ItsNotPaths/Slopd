...
test_indent_levels_through_blank :: proc(t: ^testing.T) {
    b := mkbuf("def f():\n    a = 1\n\n    b = 2\nx = 3")
    defer edit.buffer_destroy(&b)
    testing.expect_value(t, edit.buffer_indent_levels(&b, 0, 4), 0)
    testing.expect_value(t, edit.buffer_indent_levels(&b, 1, 4), 1)
    testing.expect_value(t, edit.buffer_indent_levels(&b, 2, 4), 1) // blank, borrowed
    testing.expect_value(t, edit.buffer_indent_levels(&b, 3, 4), 1)
    testing.expect_value(t, edit.buffer_indent_levels(&b, 4, 4), 0)
}
...
test_active_scope :: proc(t: ^testing.T) {
    ...
    below := edit.buffer_active_scope(&b, 1, 4, 2, 3)
...
test_fold_range_indent :: proc(t: ^testing.T) {
    ...
    s, e, ok := edit.fold_range_indent(&b, 0)
    testing.expect(t, ok)
    testing.expect_value(t, s, 0)
    testing.expect_value(t, e, 2) // both body lines, not "x = 3"

    _, _, ok2 := edit.fold_range_indent(&b, 1) // a body line opens no block
...
test_fold_visibility_helpers :: proc(t: ^testing.T) {
    ...
    testing.expect(t, !edit.buffer_line_hidden(&b, 0))
    testing.expect(t, edit.buffer_line_hidden(&b, 1))
    testing.expect(t, edit.buffer_line_hidden(&b, 2))
    testing.expect(t, !edit.buffer_line_hidden(&b, 3))

    testing.expect_value(t, edit.buffer_next_visible(&b, 1), 3)
    testing.expect_value(t, edit.buffer_prev_visible(&b, 2), 0)
    testing.expect_value(t, edit.buffer_visible_count(&b, 0, 3), 2)
    testing.expect_value(t, edit.buffer_back_visible(&b, 3, 1), 0)
    ...
    testing.expect_value(t, edit.buffer_fwd_visible(&b, 0, 1), 3)
    testing.expect_value(t, edit.buffer_fwd_visible(&b, 0, 9), 3) // past the end clamps
    testing.expect_value(t, edit.buffer_fwd_visible(&b, 3, 1), 3)

    tail := mkbuf("x = 0\ndef f():\n    a = 1\n    b = 2")
    defer edit.buffer_destroy(&tail)
    append(&tail.folds, edit.Fold{line = 1, end = 3})
    testing.expect_value(t, edit.buffer_fwd_visible(&tail, 0, 5), 1) // the header, not line 3
}
...
test_fold_toggle_motion_and_sync :: proc(t: ^testing.T) {
    ...
    app.buffer_fold_toggle(&a, b)
    testing.expect_value(t, len(b.folds), 1)
    edit.buffer_motion(b, .Down) // onto the first visible line past the fold
    edit.buffer_newline(b)
    edit.buffer_sync_folds(b)
    testing.expect_value(t, len(b.folds), 1)
    testing.expect_value(t, b.folds[0], edit.Fold{line = 0, end = 2})
}
...
test_fold_survives_edits :: proc(t: ^testing.T) {
    ...
    b := edit.editor_current(&a.editor)
    //             0         1         2          3          4          5
    edit.buffer_set_text(b, "import x\nimport y\ndef f():\n    a = 1\n    b = 2\nx = 3")

    fold_at :: proc(a: ^app.App, b: ^edit.Buffer, line: int) {
    ...
    fold_at(&a, b, 2)
    testing.expect_value(t, len(b.folds), 1)
    testing.expect_value(t, b.folds[0], edit.Fold{line = 2, end = 4})
    ...
    edit.buffer_sync_folds(b)
    testing.expect_value(t, len(b.folds), 1)
    testing.expect_value(t, b.folds[0], edit.Fold{line = 4, end = 6})
    ...
    edit.buffer_sync_folds(b)
    testing.expect_value(t, b.folds[0], edit.Fold{line = 3, end = 5})
