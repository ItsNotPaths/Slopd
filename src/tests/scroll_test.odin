...
test_list_follow :: proc(t: ^testing.T) {
    testing.expect_value(t, ui.list_scroll_target(0, 0, ROWS, TOTAL, false), 0) // sel on row 0
    testing.expect_value(t, ui.list_scroll_target(0, 5, ROWS, TOTAL, false), 0) // no move
    testing.expect_value(t, ui.list_scroll_target(0, 12, ROWS, TOTAL, false), 3) // onto the bottom row
    testing.expect_value(t, ui.list_scroll_target(3, 1, ROWS, TOTAL, false), 1) // above: top = sel
    testing.expect_value(t, ui.list_scroll_target(3, 12, ROWS, TOTAL, false), 3) // last row: still inside
}
...
test_list_middle :: proc(t: ^testing.T) {
    testing.expect_value(t, ui.list_scroll_target(0, 0, ROWS, TOTAL, true), 0) // clamped at the top
    testing.expect_value(t, ui.list_scroll_target(0, 3, ROWS, TOTAL, true), 0) // still clamped
    testing.expect_value(t, ui.list_scroll_target(0, 40, ROWS, TOTAL, true), 35)
    testing.expect_value(t, ui.list_scroll_target(35, 41, ROWS, TOTAL, true), 36) // one row = one row
    testing.expect_value(t, ui.list_scroll_target(0, 99, ROWS, TOTAL, true), 94) // past the end, centred
}
...
test_list_follow_reclamps_stale_top :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, ui.list_scroll_target(90, 15, ROWS, 20, false), 10)
    // A selection above the re-clamped top pulls it up.
    testing.expect_value(t, ui.list_scroll_target(90, 2, ROWS, 20, false), 2)
}
...
test_list_empty_and_zero_rows :: proc(t: ^testing.T) {
    testing.expect_value(t, ui.list_scroll_target(5, 3, ROWS, 0, false), 0) // empty list
    testing.expect_value(t, ui.list_scroll_target(5, 3, ROWS, 0, true), 0)
    testing.expect_value(t, ui.list_scroll_target(5, 3, 0, TOTAL, false), 0) // zero-height pane
    testing.expect_value(t, ui.list_scroll_target(5, 3, 0, TOTAL, true), 0)
}
...
test_list_shorter_than_viewport :: proc(t: ^testing.T) {
    testing.expect_value(t, ui.list_scroll_target(0, 2, ROWS, 4, false), 0)
    testing.expect_value(t, ui.list_scroll_target(3, 2, ROWS, 4, false), 0) // stale top re-clamped
    testing.expect_value(t, ui.list_scroll_target(0, 99, ROWS, 4, false), 0) // sel clamped to row 3
}
...
test_list_scroll_detach :: proc(t: ^testing.T) {
    ...
    ui.list_scroll_apply(&scroll, &detached, 40, ROWS, TOTAL, true, 0)
    ...
    ui.list_scroll_by(&scroll, &detached, 9, 100)
    testing.expect_value(t, scroll, 44)
    ui.list_scroll_apply(&scroll, &detached, 40, ROWS, TOTAL, true, 0)
    testing.expect_value(t, scroll, 44)
    ui.list_scroll_apply(&scroll, &detached, 40, ROWS, TOTAL, false, 0)
    testing.expect_value(t, scroll, 44)

    // Only input AFTER the gesture re-attaches: an older timestamp cannot be a reaction to it.
    ui.list_scroll_apply(&scroll, &detached, 40, ROWS, TOTAL, false, 99)
    testing.expect_value(t, scroll, 44)
    ui.list_scroll_apply(&scroll, &detached, 40, ROWS, TOTAL, false, 101)
...
test_list_scroll_detached_is_bounded :: proc(t: ^testing.T) {
    ...
    ui.list_scroll_by(&scroll, &detached, -50, 100)
    ui.list_scroll_apply(&scroll, &detached, 0, ROWS, TOTAL, false, 0)
    ...
    ui.list_scroll_by(&scroll, &detached, 500, 100)
    ui.list_scroll_apply(&scroll, &detached, 0, ROWS, TOTAL, false, 0)
    testing.expect_value(t, scroll, TOTAL - 1)

    ui.list_scroll_by(&scroll, &detached, 500, 100)
    ui.list_scroll_apply(&scroll, &detached, 0, ROWS, 0, false, 0)
    testing.expect_value(t, scroll, 0)
}
