...
test_color_rows_carry_their_rails :: proc(t: ^testing.T) {
    ...
    prev := app.color_preview_rect(&a, PANE)
    testing.expect_value(t, prev, app.Rect{10, 10, 180, 36}) // 2px ring + an 8px margin

    last_bottom := prev.y + prev.h
    for i in 0 ..< 4 {
        row, track := app.color_row(&a, PANE, i, LH)
    ...
    still, _ := app.color_row(&dead, PANE, 0, LH)
    a.color.live = true
    row, _ := app.color_row(&a, PANE, 0, LH)
...
test_color_click_grabs_the_row_it_landed_on :: proc(t: ^testing.T) {
    a := color_app(0, 0)
    _, track := app.color_row(&a, PANE, 0, LH)
    row1, _ := app.color_row(&a, PANE, 1, LH)

    a.mouse.x, a.mouse.y = track.x + track.w / 2, track.y - 2 // on the label line, not the rail
    press(&a)
    app.color_click(&a, PANE, LH)

    testing.expect(t, app.drag_live(&a, .Color_Slider, 0), "a press on the hue row must capture it")
...
test_color_hit_refuses_what_is_not_a_rail :: proc(t: ^testing.T) {
    a := color_app(0, 0)
    prev := app.color_preview_rect(&a, PANE)
    row2, _ := app.color_row(&a, PANE, 2, LH)
    row3, _ := app.color_row(&a, PANE, 3, LH)

    a.mouse.x, a.mouse.y = prev.x + 4, prev.y + 4
    testing.expect_value(t, app.color_hit(&a, PANE, LH), -1)

    a.mouse.x, a.mouse.y = row2.x + 4, row2.y + 4
    testing.expect_value(t, app.color_hit(&a, PANE, LH), 2)

    a.mouse.y = row3.y + 4 // no alpha channel: the fourth row does not exist
    testing.expect_value(t, app.color_hit(&a, PANE, LH), -1)
    with_alpha := color_app(a.mouse.x, a.mouse.y, true)
    testing.expect_value(t, app.color_hit(&with_alpha, PANE, LH), 3)
...
test_color_drag_only_writes_its_own_capture :: proc(t: ^testing.T) {
    ...
    a.mouse.down = true
    app.drag_begin(&a, .Editor_Text, 0, 1, app.Pos{}, 0)
    ...
    press(&outside)
    app.color_click(&outside, PANE, LH)
...
test_color_rails_show_what_they_would_do :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, app.color_track_at(&a, 0, 1.0 / 3), [3]f32{0, 1, 0}) // 120° green
    testing.expect_value(t, app.color_track_at(&a, 2, 1), [3]f32{1, 0, 0}) // full value is the hue
    testing.expect_value(t, app.color_track_at(&a, 3, 0), a.theme.bg) // alpha 0 is the pane
