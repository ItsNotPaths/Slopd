...
// The context menu declared in Clay. Two claims that no other surface in the program makes:
// it is placed by the POINTER rather than by the layout, and it must paint OVER a pane rather
// than inside one — so its clip group has to open after that pane's has closed, which is
// invisible in the boxes and is the thing this file exists to pin.
//
// With the 10x16 test font at scale 1: rows are 20px (16 + CM_ROW_PAD), a separator band is
// 5px, and the margin is 8px either side. The menu below is 12 cells wide at its widest item
// ("Open" + a 3-cell gap + "Enter"), so 136px by 65px.
...
// sized to its contents before it is placed — the one place in the chrome that measures a
// string itself, since the placement needs a width Clay has not solved yet.
@(test)
test_ctxmenu_command_list :: proc(t: ^testing.T) {
    ...
    // The separator is a band with a single top border, not a filled bar, so the rows either
    // side of it are 20px apart plus the 5px gap.
    cut, cok := box_of(&cmds, clay.ID("cm_item", 2), .Rectangle)
    testing.expect(t, !cok, "an unselected item painted a background")
    _ = cut

    // Text: the label on the margin, the hint pushed to the right edge by the solver (rule 5)
    // rather than by padding the label out to a column.
    label, hint: app.Rect
    seen := 0
    ...
    testing.expect_value(t, seen, 6) // three items of label + hint; the separator has no text
}

// **The occlusion claim.** A popup that is declared in the flow of a pane lands inside that
// pane's scissor group, where the renderer composites quads UNDER glyphs (clay_render.odin) —
// so its backdrop would paint behind the file names it is covering, in a build where every
// bounding box is correct. `floating` is what avoids it: a floating element is its own layout
// root, and Clay emits roots in order, so the whole pane subtree is on the list first.
@(test)
test_ctxmenu_paints_over_a_pane :: proc(t: ^testing.T) {
    ...
    // A pane and the popup in ONE tree, in the order window_frame declares them.
    ...
    // Walk the list tracking clip depth: the menu's backdrop must be inside a group that opened
    // after every one of the pane's had closed. A group is found by CONTAINMENT rather than by
    // id, because Clay derives a scissor's id from the element that asked for one (C8c).
    ...
    // And nothing opens a group after it: the popup is the last thing painted in the window.
    after := 0
    passed := false
    for i in 0 ..< cmds.length {
        c := clay.RenderCommandArray_Get(&cmds, i)
...
// The hit test and the verb. A single press ACTIVATES — a context menu is already the second
// half of a gesture, the right press having been the first — and a press anywhere inside the
// box is swallowed, separators and disabled rows included, rather than reaching the pane under it.
@(test)
test_ctxmenu_hit_and_click :: proc(t: ^testing.T) {
    ...
    clay.SetPointerState({50, 45}, false) // the third row: 10 + 20 + 5 + a few px
    _ = app.ctxmenu_layout(&a, &f, 600, 300)
    testing.expect_value(t, app.ctxmenu_hit(&a.ctxmenu), 2)

    // The separator band is dead space, as chrome rows are in every list pane.
    ...
    // A press on a live item runs it on the FIRST press and closes the menu with it.
    a.mouse.click = true
    ...
    // A press outside the box is not the menu's at all — ctxmenu_dismiss_click has it, a frame
    // earlier and before any pane can (window_pointer).
    menu_app(&a)
    a.ctxmenu.rect = MENU
    a.mouse.click = true
