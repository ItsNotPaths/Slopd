...
dv_variant :: proc(sig: string, v: app.Dbus_Value) -> app.Dbus_Value {
    p := new(app.Dbus_Value, context.temp_allocator)
    p^ = v
    return app.Dbus_Variant{sig = sig, val = p}
}

dv_entry :: proc(key, val: app.Dbus_Value) -> app.Dbus_Value {
    k := new(app.Dbus_Value, context.temp_allocator)
    v := new(app.Dbus_Value, context.temp_allocator)
    k^, v^ = key, val
    return app.Dbus_Entry{key = k, val = v}
}

// `sig` is the ELEMENT signature, matching Dbus_Array.
dv_arr :: proc(sig: string, items: ..app.Dbus_Value) -> app.Dbus_Value {
    return app.Dbus_Array{sig = sig, items = slice.clone(items, context.temp_allocator)}
}

dv_strs :: proc(items: ..string) -> app.Dbus_Value {
    out := make([dynamic]app.Dbus_Value, 0, len(items), context.temp_allocator)
    for s in items {
        append(&out, app.Dbus_String(s))
    }
    return app.Dbus_Array{sig = "s", items = out[:]}
}

// One `{sv}` property entry, and the `a{sv}` bag they go in.
dv_prop :: proc(name, sig: string, v: app.Dbus_Value) -> app.Dbus_Value {
    return dv_entry(app.Dbus_String(name), dv_variant(sig, v))
}

dv_props :: proc(items: ..app.Dbus_Value) -> app.Dbus_Value {
    return app.Dbus_Array{sig = "{sv}", items = slice.clone(items, context.temp_allocator)}
}

// One `{sa{sv}}` interface entry, and the `a{sa{sv}}` dict of them.
dv_iface :: proc(name: string, bag: app.Dbus_Value) -> app.Dbus_Value {
    return dv_entry(app.Dbus_String(name), bag)
}

dv_ifaces :: proc(items: ..app.Dbus_Value) -> app.Dbus_Value {
    return app.Dbus_Array{sig = "{sa{sv}}", items = slice.clone(items, context.temp_allocator)}
}
...
dv_signal :: proc(
    ...
    msg: app.Dbus_Message,
    ok: bool,
) {
    raw := app.dbus_msg_build(
        .Signal,
        7,
        path = path,
        iface = iface,
    ...
    return app.dbus_msg_decode(raw)
}
...
dv_managed_fixture :: proc() -> app.Dbus_Value {
    return app.Dbus_Array {
        sig = "{oa{sa{sv}}}",
        items = slice.clone(
            []app.Dbus_Value {
                dv_entry(
                    app.Dbus_Path(IWD_ADAPTER),
                    dv_ifaces(
                        dv_iface(
                            "net.connman.iwd.Adapter",
                            dv_props(
                                dv_prop("Powered", "b", true),
                                dv_prop("Name", "s", app.Dbus_String("phy0")),
                ...
                dv_entry(
                    app.Dbus_Path(IWD_DEVICE),
                    dv_ifaces(
                        dv_iface(
                            "net.connman.iwd.Device",
                            dv_props(
                                dv_prop("Name", "s", app.Dbus_String("wlp61s0")),
                                dv_prop("Adapter", "o", app.Dbus_Path(IWD_ADAPTER)),
                            ),
                        ),
                        dv_iface(
                            "net.connman.iwd.Station",
                            dv_props(dv_prop("State", "s", app.Dbus_String("connected"))),
                        ),
                    ),
                ),
            },
...
test_dbus_value_clone_outlives_msg :: proc(t: ^testing.T) {
    ...
    clone := app.dbus_value_clone(args[0])
    defer app.dbus_value_free(clone)
    ...
    rssi, rok := app.dbus_dict_get(clone, "Rssi")
    testing.expect(t, rok)
    n, iok := app.dbus_as_int(rssi)
...
test_dbus_om_load :: proc(t: ^testing.T) {
    om := loaded_om()
    defer app.dbus_om_destroy(om)

    testing.expect_value(t, len(om.objects), 2)
    testing.expect(t, app.dbus_om_has(om, IWD_ADAPTER, "net.connman.iwd.Adapter"))
    testing.expect(t, app.dbus_om_has(om, IWD_DEVICE, "net.connman.iwd.Station"))

    powered, pok := app.dbus_om_bool(om, IWD_ADAPTER, "net.connman.iwd.Adapter", "Powered")
    testing.expect(t, pok)
    testing.expect(t, powered)

    name, nok := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Device", "Name")
    testing.expect(t, nok)
    testing.expect_value(t, name, "wlp61s0")

    // An object path property keeps its type but reads as a string.
    adapter, aok := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Device", "Adapter")
    testing.expect(t, aok)
    testing.expect_value(t, adapter, IWD_ADAPTER)

    // Misses are misses, not zero values.
    _, missing := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Device", "NoSuchProp")
    testing.expect(t, !missing)
    _, no_iface := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Adapter", "Name")
    testing.expect(t, !no_iface)
    _, no_obj := app.dbus_om_string(om, "/nope", "net.connman.iwd.Device", "Name")
...
test_dbus_om_filter :: proc(t: ^testing.T) {
    ...
    testing.expect(
        t,
        !app.dbus_om_has(om, IWD_ADAPTER, "org.freedesktop.DBus.Properties"),
        "a filtered-out interface must not be stored",
    )

    // A nil filter keeps everything.
    all := app.dbus_om_make("net.connman.iwd", "/")
    defer app.dbus_om_destroy(all)
    app.dbus_om_load(all, dv_managed_fixture())
    testing.expect(t, app.dbus_om_has(all, IWD_ADAPTER, "org.freedesktop.DBus.Properties"))

    // An object whose every interface is filtered away never appears.
    none := app.dbus_om_make("net.connman.iwd", "/", []string{"net.connman.iwd.Nothing"})
    defer app.dbus_om_destroy(none)
    app.dbus_om_load(none, dv_managed_fixture())
...
test_dbus_om_interfaces_added :: proc(t: ^testing.T) {
    ...
    msg, mok := dv_signal(
        ...
        "oa{sa{sv}}",
        []app.Dbus_Value {
            app.Dbus_Path(net_path),
            dv_ifaces(
                dv_iface(
                    "net.connman.iwd.Network",
                    dv_props(
                        dv_prop("Name", "s", app.Dbus_String("Smith")),
    ...
    testing.expect(t, !app.dbus_om_apply(om, &msg))
    testing.expect_value(t, len(om.objects), 2)

    wide := app.dbus_om_make("net.connman.iwd", "/", []string{"net.connman.iwd.Network"})
    defer app.dbus_om_destroy(wide)
    testing.expect(t, app.dbus_om_apply(wide, &msg), "a kept interface should be taken")
    testing.expect(t, wide.dirty, "a mutation must mark the tree dirty")
    name, nok := app.dbus_om_string(wide, net_path, "net.connman.iwd.Network", "Name")
    testing.expect(t, nok)
    ...
    testing.expect(t, aok)
    testing.expect(t, app.dbus_om_apply(wide, &again))
    conn, cok := app.dbus_om_bool(wide, net_path, "net.connman.iwd.Network", "Connected")
    testing.expect(t, cok)
    testing.expect(t, conn)
    _, stale := app.dbus_om_string(wide, net_path, "net.connman.iwd.Network", "Name")
...
test_dbus_om_interfaces_removed :: proc(t: ^testing.T) {
    ...
    one, ok1 := dv_signal(
        ...
        "",
        "oas",
        []app.Dbus_Value{app.Dbus_Path(IWD_DEVICE), dv_strs("net.connman.iwd.Station")},
    )
    testing.expect(t, ok1)
    testing.expect(t, app.dbus_om_apply(om, &one))
    testing.expect(t, !app.dbus_om_has(om, IWD_DEVICE, "net.connman.iwd.Station"))
    testing.expect(t, app.dbus_om_has(om, IWD_DEVICE, "net.connman.iwd.Device"))
    testing.expect_value(t, len(om.objects), 2)
    ...
    rest, ok2 := dv_signal(
        ...
        "",
        "oas",
        []app.Dbus_Value{app.Dbus_Path(IWD_DEVICE), dv_strs("net.connman.iwd.Device")},
    )
    testing.expect(t, ok2)
    testing.expect(t, app.dbus_om_apply(om, &rest))
...
test_dbus_om_properties_changed :: proc(t: ^testing.T) {
    ...
    msg, mok := dv_signal(
        ...
        []app.Dbus_Value {
            ...
            dv_props(
                dv_prop("State", "s", app.Dbus_String("disconnected")),
    ...
    state, sok := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Station", "State")
    testing.expect(t, sok)
    testing.expect_value(t, state, "disconnected") // replaced, and the old string freed
    scanning, cok := app.dbus_om_bool(om, IWD_DEVICE, "net.connman.iwd.Station", "Scanning")
    ...
    testing.expect(t, app.dbus_om_apply(om, &inv))
    _, still := app.dbus_om_string(om, IWD_DEVICE, "net.connman.iwd.Station", "State")
    ...
    ghost_iface, gok := dv_signal(
        ...
        "sa{sv}as",
        []app.Dbus_Value {
            app.Dbus_String("net.connman.iwd.NeverAnnounced"),
            dv_props(dv_prop("X", "b", true)),
            dv_strs(),
        },
    )
    testing.expect(t, gok)
    testing.expect(t, !app.dbus_om_apply(om, &ghost_iface))
    ...
    testing.expect(t, !app.dbus_om_apply(om, &ghost_obj))
    testing.expect_value(t, len(om.objects), 2)
}
...
test_dbus_om_name_owner_changed :: proc(t: ^testing.T) {
    om := loaded_om()
    defer app.dbus_om_destroy(om)
    app.dbus_om_set_owner(om, "") // dbus_om_make leaves it empty; be explicit about it
    ...
    other, ok1 := dv_signal(
        ...
        "sss",
        []app.Dbus_Value {
            app.Dbus_String("org.bluez"),
            app.Dbus_String(""),
            app.Dbus_String(":1.9"),
        },
    )
    testing.expect(t, ok1)
    testing.expect(t, !app.dbus_om_apply(om, &other))
    testing.expect_value(t, len(om.objects), 2)
    ...
    testing.expect(t, ok2)
    testing.expect(t, app.dbus_om_apply(om, &gone))
    testing.expect_value(t, len(om.objects), 0)
    testing.expect(t, !app.dbus_om_present(om))
    ...
    testing.expect(t, app.dbus_om_apply(om, &back))
    testing.expect(t, app.dbus_om_present(om))
...
test_dbus_om_sender_scope :: proc(t: ^testing.T) {
    ...
    om := loaded_om()
    defer app.dbus_om_destroy(om)
    app.dbus_om_set_owner(om, ":1.7")

    body := []app.Dbus_Value {
        app.Dbus_String("net.connman.iwd.Station"),
        dv_props(dv_prop("State", "s", app.Dbus_String("scanning"))),
        dv_strs(),
    }
    ...
    testing.expect(t, ok1)
    testing.expect(t, app.dbus_om_apply(om, &from_owner), "the unique name is the real sender")
    ...
    testing.expect(t, ok2)
    testing.expect(t, app.dbus_om_apply(om, &from_wellknown), "the well-known name is accepted too")
    ...
    testing.expect(t, ok3)
    testing.expect(t, !app.dbus_om_apply(om, &imposter), "another peer must not write our tree")
}
...
test_dbus_om_rejects_malformed :: proc(t: ^testing.T) {
    ...
    call_raw, cok := app.dbus_msg_build(
        .Method_Call,
        1,
        path = "/",
        ...
        body_sig = "oas",
        args = []app.Dbus_Value{app.Dbus_Path(IWD_DEVICE), dv_strs("net.connman.iwd.Station")},
        alloc = context.temp_allocator,
    )
    testing.expect(t, cok)
    call, dok := app.dbus_msg_decode(call_raw)
    testing.expect(t, dok)
    testing.expect(t, !app.dbus_om_apply(om, &call))
    ...
    wrong_sig, wok := dv_signal(
        ...
        "",
        "os",
        []app.Dbus_Value{app.Dbus_Path(IWD_DEVICE), app.Dbus_String("net.connman.iwd.Station")},
    )
    testing.expect(t, wok)
    testing.expect(t, !app.dbus_om_apply(om, &wrong_sig))
    ...
    testing.expect(t, rok)
    testing.expect(t, !app.dbus_om_apply(om, &wrong_root))
    ...
    testing.expect(t, !app.dbus_om_apply(om, &unknown))
...
test_dbus_om_scope :: proc(t: ^testing.T) {
    ...
    om := app.dbus_om_make("net.connman.iwd", "/net/connman/iwd", []string{"t.T"})
    defer app.dbus_om_destroy(om)

    accept := app.dbus_om_put(
        om,
        "/net/connman/iwd/0",
    ...
    for bad in ([]string{"/net/connman/iwdX", "/net/connman", "/", "relative", ""}) {
        testing.expectf(
            t,
            !app.dbus_om_put(om, bad, dv_ifaces(dv_iface("t.T", dv_props()))),
...
test_dbus_om_live_iwd :: proc(t: ^testing.T) {
    ...
    adapters := app.dbus_om_paths(om, "net.connman.iwd.Adapter")
    ...
    _, pok := app.dbus_om_bool(om, adapters[0], "net.connman.iwd.Adapter", "Powered")
    ...
    for path in app.dbus_om_paths(om) {
        obj, fok := app.dbus_om_object(om, path)
