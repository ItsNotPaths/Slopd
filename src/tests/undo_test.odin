...
test_undo_coalesce :: proc(t: ^testing.T) {
    ...
    testing.expect(t, !txt.doc_undo(&d)) // nothing left

    testing.expect(t, txt.doc_redo(&d))
    testing.expect_value(t, ln(&d, 0), "abc")
...
test_undo_break_on_move :: proc(t: ^testing.T) {
    ...
    defer txt.doc_destroy(&d)
    typ(&d, "ab")
    txt.doc_move(&d, .Left) // caret no longer where typing left off
    typ(&d, "c") // -> "acb"
...
test_undo_delete :: proc(t: ^testing.T) {
    d := mkdoc("abc")
    defer txt.doc_destroy(&d)
    txt.doc_move(&d, .End) // {0,3}
    txt.doc_backspace(&d) // "ab"
...
test_undo_multicursor_coalesce :: proc(t: ^testing.T) {
    ...
    testing.expect(t, txt.doc_undo(&d)) // one step reverts both runs
...
test_undo_across_multicursor :: proc(t: ^testing.T) {
    ...
    txt.doc_drop_anchor(&d) // anchor at the caret {0,1}
    txt.doc_move(&d, .Down) // free caret -> {1,1}, anchor stays {0,1}
    typ(&d, "Y") // "XYab" / "cYd"  (multi-cursor step)
    testing.expect_value(t, ln(&d, 0), "XYab")
    testing.expect_value(t, ln(&d, 1), "cYd")

    testing.expect(t, txt.doc_undo(&d)) // revert the multi-cursor Y
    testing.expect_value(t, ln(&d, 0), "Xab")
    testing.expect_value(t, ln(&d, 1), "cd")
    testing.expect(t, txt.doc_undo(&d)) // revert the single-cursor X
    testing.expect_value(t, ln(&d, 0), "ab")
    testing.expect_value(t, ln(&d, 1), "cd")
}
...
test_undo_redo_cleared_on_edit :: proc(t: ^testing.T) {
    ...
    defer txt.doc_destroy(&d)
    typ(&d, "ab")
    txt.doc_undo(&d) // ""  (redo now holds "ab")
