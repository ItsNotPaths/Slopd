...
test_pair_skip_closer :: proc(t: ^testing.T) {
    b := mkbuf("()")
    defer app.buffer_destroy(&b)
    b.cursors[0] = app.Cursor{head = {0, 1}, anchor = {0, 1}} // between ( )
...
test_quote_after_word_is_literal :: proc(t: ^testing.T) {
    b := mkbuf("don")
    defer app.buffer_destroy(&b)
    b.cursors[0] = app.Cursor{head = {0, 3}, anchor = {0, 3}} // after "don"
    app.buffer_autopair(&b, '\'')
...
test_pair_backspace_empty :: proc(t: ^testing.T) {
    b := mkbuf("()")
    defer app.buffer_destroy(&b)
    b.cursors[0] = app.Cursor{head = {0, 1}, anchor = {0, 1}}
...
test_pair_surround_selection :: proc(t: ^testing.T) {
    b := mkbuf("foo")
    defer app.buffer_destroy(&b)
    b.cursors[0] = app.Cursor{head = {0, 3}, anchor = {0, 0}} // select "foo"
    app.buffer_autopair(&b, '(')
    testing.expect_value(t, bln(&b, 0), "(foo)")
}
...
test_tab_skips_out_of_pair :: proc(t: ^testing.T) {
    b := mkbuf("(stuff)")
    defer app.buffer_destroy(&b)
    b.cursors[0] = app.Cursor{head = {0, 6}, anchor = {0, 6}} // just before )
...
test_pair_multicursor :: proc(t: ^testing.T) {
    ...
    defer app.buffer_destroy(&b)
    clear(&b.cursors)
    append(&b.cursors, app.Cursor{head = {0, 1}, anchor = {0, 1}}) // end of line 0
    append(&b.cursors, app.Cursor{head = {1, 1}, anchor = {1, 1}}) // end of line 1
    app.buffer_autopair(&b, '(')
