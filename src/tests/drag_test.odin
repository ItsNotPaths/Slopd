...
test_drag_begin_needs_a_held_button :: proc(t: ^testing.T) {
    ...
    ui.drag_begin(app.ctx_of(&a), .Editor_Text, 2, 3, txt.Pos{7, 4}, 3)
    testing.expect_value(t, a.drag.kind, ui.Drag_Kind.Editor_Text)
    testing.expect_value(t, a.drag.target, 2)
    testing.expect_value(t, a.drag.grade, 3)
    testing.expect_value(t, a.drag.anchor, txt.Pos{7, 4})
    ...
    b.mouse.down = false
    ui.drag_begin(app.ctx_of(&b), .Editor_Text, 0, 1, txt.Pos{1, 1}, 1)
...
test_drag_live_is_kind_and_target :: proc(t: ^testing.T) {
    a := dragging_app(120, 90)
    ui.drag_begin(app.ctx_of(&a), .Editor_Text, 1, 1, txt.Pos{0, 0}, 0)

    testing.expect(t, ui.drag_live(app.ctx_of(&a), .Editor_Text, 1), "the drag it began")
    testing.expect(t, !ui.drag_live(app.ctx_of(&a), .Editor_Text, 0), "another buffer must not be written")
    testing.expect(t, !ui.drag_live(app.ctx_of(&a), .Terminal_Sel, 1), "another kind of drag entirely")
}
...
test_drag_release_parks_then_sweeps :: proc(t: ^testing.T) {
    a := dragging_app(120, 90)
    ui.drag_begin(app.ctx_of(&a), .Editor_Text, 0, 1, txt.Pos{2, 2}, 2)

    // A sweep before the button comes up changes nothing.
    ui.drag_sweep(app.ctx_of(&a))
    testing.expect(t, ui.drag_live(app.ctx_of(&a), .Editor_Text, 0), "a live drag must survive its own frames")

    a.mouse.down = false
    ui.drag_release(app.ctx_of(&a))
    testing.expect(t, a.drag.ending, "the release must be recorded")
    testing.expect(t, ui.drag_live(app.ctx_of(&a), .Editor_Text, 0), "the owed frame: still live after the release")
...
test_drag_sweep_buries_on_mouse_off :: proc(t: ^testing.T) {
    a := dragging_app(120, 90)
    ui.drag_begin(app.ctx_of(&a), .Editor_Text, 0, 1, txt.Pos{0, 0}, 0)
...
test_drag_autoscrolling_edges :: proc(t: ^testing.T) {
    ...
    b := dragging_app(0, 0)
    b.lay = {}
    ui.drag_begin(app.ctx_of(&b), .Editor_Text, 0, 1, txt.Pos{0, 0}, 0)
...
test_drag_scroll_step :: proc(t: ^testing.T) {
    testing.expect_value(t, ui.drag_scroll_step(0, 18), 1)
    testing.expect_value(t, ui.drag_scroll_step(17, 18), 1)
    testing.expect_value(t, ui.drag_scroll_step(18, 18), 2)
    testing.expect_value(t, ui.drag_scroll_step(54, 18), 4)
    testing.expect_value(t, ui.drag_scroll_step(100000, 18), ui.DRAG_SCROLL_MAX)
    testing.expect_value(t, ui.drag_scroll_step(50, 0), 1) // a degenerate pane still steps
}
...
test_drag_tick_paces_and_starts_immediately :: proc(t: ^testing.T) {
    ...
    testing.expect(t, ui.drag_tick(app.ctx_of(&a), 100), "the first tick is due at once")
    testing.expect(t, !ui.drag_tick(app.ctx_of(&a), 100), "and is spent: no second step in the same frame")
    testing.expect(t, !ui.drag_tick(app.ctx_of(&a), 100 + ui.DRAG_SCROLL_S/2), "nor half an interval later")
    testing.expect(t, ui.drag_tick(app.ctx_of(&a), 100 + ui.DRAG_SCROLL_S), "the next interval steps again")
}
...
test_drag_next_wake :: proc(t: ^testing.T) {
    ...
    _ = ui.drag_tick(app.ctx_of(&a), 100)
    // A whole interval, give or take the f64 round trip through an absolute glfw time.
    testing.expect(t, abs(app.drag_next_wake(&a, 100) - ui.DRAG_SCROLL_S) < 1e-9, "one interval to the next step")
    testing.expect_value(t, app.drag_next_wake(&a, 100 + ui.DRAG_SCROLL_S), 0.0)
}
...
test_drag_moved_latches :: proc(t: ^testing.T) {
    ...
    ui.drag_begin(app.ctx_of(&a), .Split, 0, 1, txt.Pos{}, 0)

    testing.expect(t, !ui.drag_moved(app.ctx_of(&a), 3), "a press that has not moved is not a drag")
    a.mouse.x = 302 // two pixels: inside the threshold
    testing.expect(t, !ui.drag_moved(app.ctx_of(&a), 3), "nor is one that has barely twitched")

    a.mouse.x = 340
    testing.expect(t, ui.drag_moved(app.ctx_of(&a), 3), "past the threshold it is a drag")
    a.mouse.x = 301 // back within a pixel of the press
    testing.expect(t, ui.drag_moved(app.ctx_of(&a), 3), "and stays one on the way back")

    // Max-norm: either axis alone is enough.
    b := dragging_app(300, 200)
    ui.drag_begin(app.ctx_of(&b), .Media_Pan, 0, 1, txt.Pos{}, 0)
    b.mouse.y = 210
    testing.expect(t, ui.drag_moved(app.ctx_of(&b), 3), "vertical travel counts too")

    // A fresh capture starts un-latched: the field is per-gesture.
    ui.drag_release(app.ctx_of(&b))
    ui.drag_sweep(app.ctx_of(&b))
    ui.drag_begin(app.ctx_of(&b), .Split, 0, 1, txt.Pos{}, 0)
    testing.expect(t, !ui.drag_moved(app.ctx_of(&b), 3), "a new press starts from nothing")
}
...
test_drag_begin_carries_the_pan_origin :: proc(t: ^testing.T) {
    ...
    ui.drag_begin(app.ctx_of(&a), .Media_Pan, 0, 1, txt.Pos{}, 0, [2]f32{-30, 12})

    testing.expect_value(t, a.drag.kind, ui.Drag_Kind.Media_Pan)
    ...
    ui.drag_begin(app.ctx_of(&b), .Editor_Text, 0, 1, txt.Pos{3, 3}, 3)
