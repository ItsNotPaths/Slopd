...
test_doc_word_motion :: proc(t: ^testing.T) {
    ...
    txt.doc_move(&d, .Word_Right);l, c := head(&d);testing.expect_value(t, c, 3) // after foo
    txt.doc_move(&d, .Word_Right);l, c = head(&d);testing.expect_value(t, c, 4) // after .
    txt.doc_move(&d, .Word_Right);l, c = head(&d);testing.expect_value(t, c, 7) // after bar
    txt.doc_move(&d, .Word_Right);l, c = head(&d);testing.expect_value(t, c, 11) // after baz
    txt.doc_move(&d, .Word_Left);l, c = head(&d);testing.expect_value(t, c, 8) // start of baz
    _ = l
}
...
test_doc_move_collapses_selection :: proc(t: ^testing.T) {
    ...
    txt.doc_move(&d, .Home)
    for _ in 0 ..< 3 {
        txt.doc_move(&d, .Right, true) // select "abc": head 3, anchor 0
    }
    txt.doc_move(&d, .Left) // plain move collapses to the left edge
    testing.expect(t, !txt.cursor_has_selection(d.cursors[0]))
...
test_doc_delete_word_back :: proc(t: ^testing.T) {
    ...
    defer txt.doc_destroy(&d)
    txt.doc_move(&d, .End)
    txt.doc_delete_word_back(&d) // removes "beta"
    testing.expect_value(t, dline(&d, 0), "alpha ")
    txt.doc_delete_word_back(&d) // removes " alpha" back to start
...
test_doc_newline_and_join :: proc(t: ^testing.T) {
    ...
    txt.doc_move(&d, .Right) // col 2
    txt.doc_newline(&d) // split into "he" / "llo"
    testing.expect_value(t, txt.doc_line_count(&d), 2)
    ...
    txt.doc_backspace(&d) // at col 0 -> join back
...
test_doc_multi_cursor_same_line :: proc(t: ^testing.T) {
    ...
    defer txt.doc_destroy(&d)
    clear(&d.cursors)
    append(&d.cursors, txt.Cursor{head = {0, 1}, anchor = {0, 1}})
    append(&d.cursors, txt.Cursor{head = {0, 3}, anchor = {0, 3}})

    txt.doc_insert_rune(&d, 'X') // -> "aXbcXd"
...
test_doc_multi_cursor_newline :: proc(t: ^testing.T) {
    ...
    clear(&d.cursors)
    append(&d.cursors, txt.Cursor{head = {0, 1}, anchor = {0, 1}})
    append(&d.cursors, txt.Cursor{head = {1, 1}, anchor = {1, 1}})

    txt.doc_newline(&d) // -> "a" / "b" / "c" / "d"
    testing.expect_value(t, txt.doc_line_count(&d), 4)
...
test_doc_drop_anchor :: proc(t: ^testing.T) {
    ...
    txt.doc_drop_anchor(&d) // anchor at {0,0}, coincident with the caret
    txt.doc_move(&d, .Down) // free caret -> {1,0}; the anchor stays at {0,0}
    txt.doc_move(&d, .Down) // free caret -> {2,0}
    testing.expect_value(t, len(d.cursors), 2)
    ...
    txt.doc_collapse_to_primary(&d) // Esc: keep the caret
...
test_doc_select_all :: proc(t: ^testing.T) {
    d := mkdoc("abc\ndefg\nhi")
    defer txt.doc_destroy(&d)
    txt.doc_reset_cursor(&d, txt.Pos{1, 2})
    txt.doc_drop_anchor(&d) // a trail: select-all is one selection whatever it finds

    txt.doc_select_all(&d)
    testing.expect_value(t, len(d.cursors), 1)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{0, 0})
    testing.expect_value(t, d.cursors[0].head, txt.Pos{2, 2})

    txt.doc_insert_rune(&d, 'x') // the selection is what an edit replaces
    testing.expect_value(t, txt.doc_line_count(&d), 1)
...
test_doc_select_lines :: proc(t: ^testing.T) {
    d := mkdoc("abc\ndefg\nhi")
    defer txt.doc_destroy(&d)
    txt.doc_reset_cursor(&d, txt.Pos{0, 1})
    txt.doc_add_cursor(&d, txt.Pos{1, 3})
    txt.doc_add_cursor(&d, txt.Pos{1, 0}) // the second line twice

    txt.doc_select_lines(&d)
    ...
    testing.expect_value(t, d.cursors[1].head, txt.Pos{1, 4})

    txt.doc_delete(&d) // both lines' text goes, the breaks stay
    testing.expect_value(t, txt.doc_line_count(&d), 3)
    testing.expect_value(t, dline(&d, 0), "")
...
test_doc_select_delete :: proc(t: ^testing.T) {
    ...
    txt.doc_move(&d, .Right, true) // select "he"
    testing.expect(t, txt.cursor_has_selection(d.cursors[0]))
    txt.doc_backspace(&d) // deletes the selection
...
test_doc_select_multiline_delete :: proc(t: ^testing.T) {
    d := mkdoc("abc\ndef")
    defer txt.doc_destroy(&d)
    txt.doc_move(&d, .Right) // {0,1}
    txt.doc_move(&d, .Right, true) // select "b" -> head {0,2}
    txt.doc_move(&d, .Down, true) // extend down to {1,2}: selection "bc\nde"
    txt.doc_delete(&d)
    testing.expect_value(t, txt.doc_line_count(&d), 1)
...
test_doc_move_all :: proc(t: ^testing.T) {
    ...
    txt.doc_move_all(&d, .Right, true)
    txt.doc_move_all(&d, .Right, true) // both select two chars
    testing.expect(t, txt.cursor_has_selection(d.cursors[0]))
    testing.expect(t, txt.cursor_has_selection(d.cursors[1]))

    txt.doc_insert_rune(&d, 'X') // replaces both selections
...
test_doc_coincident_edit :: proc(t: ^testing.T) {
    d := mkdoc("ab")
    defer txt.doc_destroy(&d)
    txt.doc_move(&d, .Right) // caret at {0,1}
    txt.doc_drop_anchor(&d) // anchor at {0,1} too (coincident)
    testing.expect_value(t, len(d.cursors), 2)

    txt.doc_insert_rune(&d, 'X') // one insert, not two
...
test_doc_pointer_cursors :: proc(t: ^testing.T) {
    ...
    txt.doc_set_head(&d, txt.Pos{0, 2}, false)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{0, 2})
    txt.doc_set_head(&d, txt.Pos{1, 3}, true)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{0, 2})
    testing.expect_value(t, d.cursors[0].head, txt.Pos{1, 3})
    ...
    txt.doc_set_head(&d, txt.Pos{99, 99}, false)
    testing.expect_value(t, d.cursors[0].head, txt.Pos{1, 7}) // last line, its length
    txt.doc_set_head(&d, txt.Pos{-4, -4}, false)
    ...
    txt.doc_add_cursor(&d, txt.Pos{1, 2})
    testing.expect_value(t, len(d.cursors), 2)
    testing.expect_value(t, d.cursors[d.primary].head, txt.Pos{1, 2})

    // The run containing the position, head at its end so a following extend grows forward.
    txt.doc_select_word(&d, txt.Pos{0, 8}) // inside "bravo"
    testing.expect_value(t, len(d.cursors), 1)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{0, 6})
    testing.expect_value(t, d.cursors[0].head, txt.Pos{0, 11})

    // Exactly what Home then Shift+End selects: the break is not included.
    txt.doc_select_line(&d, 1)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{1, 0})
    testing.expect_value(t, d.cursors[0].head, txt.Pos{1, 7})
    txt.doc_select_line(&d, 99) // clamped, not out of bounds
...
test_doc_drag_span :: proc(t: ^testing.T) {
    ...
    txt.doc_init(&d)
    defer txt.doc_destroy(&d)
    txt.doc_set_text(&d, "alpha bravo\ncharlie delta")

    // Word grade, forward: the start of the pressed run to the end of the pointed-at one.
    anchor, head := txt.doc_drag_span(&d, 2, txt.Pos{0, 7}, txt.Pos{1, 10}) // "bravo" -> "delta"
    testing.expect_value(t, anchor, txt.Pos{0, 6})
    testing.expect_value(t, head, txt.Pos{1, 13})

    // Backward past the press: the anchor flips to the far END of "bravo".
    anchor, head = txt.doc_drag_span(&d, 2, txt.Pos{0, 7}, txt.Pos{0, 2}) // -> "alpha"
    testing.expect_value(t, anchor, txt.Pos{0, 11})
    testing.expect_value(t, head, txt.Pos{0, 0})

    // Held still inside the pressed word, it is what the double click alone gives.
    anchor, head = txt.doc_drag_span(&d, 2, txt.Pos{0, 7}, txt.Pos{0, 8})
    testing.expect_value(t, anchor, txt.Pos{0, 6})
    testing.expect_value(t, head, txt.Pos{0, 11})

    // Line grade compares LINES: dragging left within the pressed line has not reversed the
    // gesture, so this stays forward.
    anchor, head = txt.doc_drag_span(&d, 3, txt.Pos{1, 9}, txt.Pos{1, 1})
    testing.expect_value(t, anchor, txt.Pos{1, 0})
    testing.expect_value(t, head, txt.Pos{1, 13})

    // …and upward it is whole lines the other way about.
    anchor, head = txt.doc_drag_span(&d, 3, txt.Pos{1, 9}, txt.Pos{0, 3})
    testing.expect_value(t, anchor, txt.Pos{1, 13})
    testing.expect_value(t, head, txt.Pos{0, 0})
    ...
    anchor, head = txt.doc_drag_span(&d, 3, txt.Pos{99, 99}, txt.Pos{-5, -5})
    testing.expect_value(t, anchor, txt.Pos{1, 13})
    testing.expect_value(t, head, txt.Pos{0, 0})

    // doc_select_span keeps the gesture's order rather than normalising it, so the head stays
    // the end the eye is at.
    txt.doc_select_span(&d, txt.Pos{1, 5}, txt.Pos{0, 2})
    testing.expect_value(t, len(d.cursors), 1)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{1, 5})
    ...
    testing.expect_value(t, lo, txt.Pos{0, 2}) // ordered on READ
    testing.expect_value(t, hi, txt.Pos{1, 5})
}
...
test_doc_multibyte_roundtrip :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, txt.doc_string(&d, context.temp_allocator), "héllo → wörld\nsecond ✓ line")
    testing.expect_value(t, txt.doc_line_len(&d, 0), 17) // BYTES

    testing.expect_value(t, txt.doc_text(&d, txt.Pos{0, 7}, txt.Pos{0, 10}, context.temp_allocator), "→")

    // Crossing the break: tail of line 0, the newline, head of line 1.
    testing.expect_value(t, txt.doc_text(&d, txt.Pos{0, 11}, txt.Pos{1, 6}, context.temp_allocator), "wörld\nsecond")

    // 13 cells over 17 bytes, and the two conversions are inverses on a boundary.
    cells := txt.doc_cells(&d, 0, context.temp_allocator)
    testing.expect_value(t, txt.cells_count(cells), 13)
    testing.expect_value(t, txt.doc_cell_col(&d, txt.Pos{0, 11}), 8) // the 'w'
    testing.expect_value(t, txt.doc_byte_col(&d, 0, 8), 11)
    testing.expect_value(t, txt.cells_off(cells, 6), 7) // the arrow
    testing.expect_value(t, txt.cells_col(cells, 7), 6)

    // A column landing inside a rune snaps back to its start: the one gate, doc_clamp_pos.
    testing.expect_value(t, txt.doc_clamp_pos(&d, txt.Pos{0, 8}), txt.Pos{0, 7})
    testing.expect_value(t, txt.doc_clamp_pos(&d, txt.Pos{0, 9}), txt.Pos{0, 7})

    // Within the line, where the splice replaces the rebuild.
    txt.doc_reset_cursor(&d, txt.Pos{0, 1})
    txt.doc_insert_rune(&d, 'ü')
    testing.expect_value(t, dline(&d, 0), "hüéllo → wörld")
    txt.doc_backspace(&d)
    testing.expect_value(t, dline(&d, 0), "héllo → wörld")
    testing.expect_value(t, txt.doc_line_count(&d), 2) // a same-line edit leaves the array
}
...
test_doc_normalizes_on_load :: proc(t: ^testing.T) {
    lf := mkdoc("a\nb\n")
    defer txt.doc_destroy(&lf)
    testing.expect_value(t, txt.doc_line_count(&lf), 2)
    testing.expect_value(t, txt.doc_string(&lf, context.temp_allocator), "a\nb")

    crlf := mkdoc("a\r\nb\r\n")
    defer txt.doc_destroy(&crlf)
    testing.expect_value(t, txt.doc_line_count(&crlf), 2)
    testing.expect_value(t, txt.doc_string(&crlf, context.temp_allocator), "a\nb")
    ...
    bare := mkdoc("a\rb")
    defer txt.doc_destroy(&bare)
    testing.expect_value(t, txt.doc_line_count(&bare), 1)
    testing.expect_value(t, txt.doc_string(&bare, context.temp_allocator), "a\rb")
}
...
test_doc_vertical_goal_is_a_cell :: proc(t: ^testing.T) {
    d := mkdoc("ééé abc\nxxxxxxx")
    defer txt.doc_destroy(&d)
    testing.expect_value(t, txt.doc_line_len(&d, 0), 10)
    testing.expect_value(t, txt.doc_cell_count(&d, 0), 7)

    txt.doc_reset_cursor(&d, txt.Pos{0, 8}) // the 'b': byte 8, cell 5
    ...
    txt.doc_move(&d, .Up) // back under the same glyph it left
    l, c = head(&d)
    testing.expect_value(t, l, 0)
    testing.expect_value(t, c, 8)

    // Past the end of a shorter line the goal is kept for the next step.
    txt.doc_reset_cursor(&d, txt.Pos{1, 7}) // cell 7, past the end of line 0's 7
    txt.doc_move(&d, .Up)
    l, c = head(&d)
    testing.expect_value(t, l, 0)
...
test_doc_home_toggles_at_the_indentation :: proc(t: ^testing.T) {
    ...
    txt.doc_move(&d, .Home) // to 0
    txt.doc_move(&d, .End)
    txt.doc_move(&d, .Home, true)
    testing.expect(t, txt.cursor_has_selection(d.cursors[0]))
    _, c = head(&d);testing.expect_value(t, c, 4)

    bare := mkdoc("   ")
    defer txt.doc_destroy(&bare)
    txt.doc_move(&bare, .Home)
...
test_doc_start_and_end_motions :: proc(t: ^testing.T) {
    ...
    txt.doc_reset_cursor(&d, txt.Pos{1, 1})
    txt.doc_move(&d, .Doc_End)
    l, c := head(&d);testing.expect_value(t, l, 2);testing.expect_value(t, c, 5)

    txt.doc_move(&d, .Doc_Start)
    ...
    txt.doc_move(&d, .Doc_End, true)
    testing.expect_value(t, d.cursors[0].anchor, txt.Pos{0, 0})
    testing.expect_value(t, d.cursors[0].head, txt.Pos{2, 5})
}
