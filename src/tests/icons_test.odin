...
test_icon_runes_are_in_the_embedded_face :: proc(t: ^testing.T) {
    ...
    info: stbtt.fontinfo
    testing.expect(t, bool(stbtt.InitFont(&info, raw_data(app.ICONS_TTF), 0)), "the icon face does not parse")
    ...
    testing.expect(t, stbtt.FindGlyphIndex(&info, app.ICON_REF) != 0, "ICON_REF is not in the face")

    missing := 0
    for r in app.ICON_BY_NAME {
        if stbtt.FindGlyphIndex(&info, r.icon) == 0 {
            missing += 1
            testing.expectf(t, false, "name rule %q maps to U+%04X, which the face lacks", r.key, r.icon)
        }
    }
    for r in app.ICON_BY_EXT {
        if stbtt.FindGlyphIndex(&info, r.icon) == 0 {
            missing += 1
    ...
    testing.expect(t, stbtt.FindGlyphIndex(&info, app.ICON_FOLDER) != 0, "no folder icon")
    testing.expect(t, stbtt.FindGlyphIndex(&info, app.ICON_FILE) != 0, "no generic file icon")
}
...
test_file_icon_mapping :: proc(t: ^testing.T) {
    testing.expect_value(t, app.file_icon("src", true), app.ICON_FOLDER)
    testing.expect_value(t, app.file_icon("Makefile", true), app.ICON_FOLDER) // a dir named like a file

    // The name table is matched first, and case-insensitively — `MAKEFILE` and `Dockerfile`
    // are both real spellings in the wild.
    testing.expect_value(t, app.file_icon("Makefile", false), app.ICON_MAKEFILE)
    testing.expect_value(t, app.file_icon("MAKEFILE", false), app.ICON_MAKEFILE)
    testing.expect_value(t, app.file_icon("Dockerfile", false), app.ICON_DOCKER)
    testing.expect_value(t, app.file_icon(".gitignore", false), app.ICON_GITIGNORE)
    testing.expect_value(t, app.file_icon("LICENSE", false), app.ICON_LICENSE)
    testing.expect_value(t, app.file_icon("Cargo.toml", false), app.ICON_RUST) // name beats .toml
    testing.expect_value(t, app.file_icon("other.toml", false), app.ICON_TOML)

    // Extensions, lower-cased.
    testing.expect_value(t, app.file_icon("main.rs", false), app.ICON_RUST)
    testing.expect_value(t, app.file_icon("main.c", false), app.ICON_C)
    testing.expect_value(t, app.file_icon("main.CPP", false), app.ICON_CPP)
    testing.expect_value(t, app.file_icon("photo.JPEG", false), app.ICON_IMAGE)
    testing.expect_value(t, app.file_icon("a.tar.gz", false), app.ICON_ARCHIVE) // the LAST extension
    testing.expect_value(t, app.file_icon("notes.md", false), app.ICON_MARKDOWN)
    ...
    testing.expect_value(t, app.file_icon(".bashrc", false), app.ICON_FILE)
    ...
    testing.expect_value(t, app.file_icon("app.odin", false), app.ICON_FILE)
    testing.expect_value(t, app.file_icon("mystery.qqq", false), app.ICON_FILE)
    testing.expect_value(t, app.file_icon("noext", false), app.ICON_FILE)
    testing.expect_value(t, app.file_icon("", false), app.ICON_FILE)
}
...
test_icon_tables_have_no_duplicate_keys :: proc(t: ^testing.T) {
    for r, i in app.ICON_BY_NAME {
        for s, j in app.ICON_BY_NAME {
            if i < j && r.key == s.key {
                testing.expectf(t, false, "duplicate name rule %q", r.key)
            }
        }
    }
    for r, i in app.ICON_BY_EXT {
        for s, j in app.ICON_BY_EXT {
            if i < j && r.key == s.key {
