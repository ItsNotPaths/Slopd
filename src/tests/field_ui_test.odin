...
// The shared one-line text field: the config pane's search and free-text rows, and the file
// browser's path line. What is testable without a window is the WINDOW — which runes show and
// where a column falls — and that is the half the path line added, so it is pinned here rather
// than in either pane's tests.

// A field cut at the head, with a window that follows the caret and moves as little as it can.
// A window re-aimed on every keystroke would slide the whole line sideways under a caret walking
// back through it.
@(test)
test_field_window :: proc(t: ^testing.T) {
    ...
    // Opened with the caret at the end: the TAIL shows, one cell held back for the '…' and one
    // for the caret past the last rune — so the caret lands on the box's last column.
    off := app.field_scroll(0, N, N, W)
    testing.expect_value(t, off, 22)
    testing.expect_value(t, app.field_col(off, N), 19)

    // Walking left inside the window does not move it; walking off the left edge moves it by
    // exactly what it takes to keep the caret on screen.
    ...
    // A press resolves a COLUMN back to a rune, which is the same sum rearranged — so a click
    // lands on the character it points at under a cut head as well as without one.
    ...
    // A box too narrow to hold an ellipsis, a caret and a rune shows the head and clips.
    testing.expect_value(t, app.field_scroll(5, N, N, 2), 0)

    // The cell count is the box divided by the advance, and a zero advance is not divided by.
...
// A field, its Doc and the box the pointer resolves against: a 20-cell box at x = 100, the
// synthetic font's 10px cell. Torn down by the caller.
@(private = "file")
...
// The pointer half: a press is a CAPTURE at a fixed grade, and every motion until the button
// comes up extends from it — the editor's gesture, one line deep.
@(test)
test_field_press_and_drag :: proc(t: ^testing.T) {
    ...
    // A single press places the caret and captures. The BOUNDARY is rounded: half a cell past
    // the fourth rune is the boundary after it.
    ...
    // Motion extends the selection from the press, wherever the pointer has since gone.
    a.mouse.x = 100 + 95
    app.field_drag(&a, f, 0)
    testing.expect_value(t, d.cursors[0].anchor.col, 5)
    testing.expect_value(t, d.cursors[0].head.col, 10)

    // Past the box's right edge the column keeps counting, so a drag off the end selects to the
    // end of the VALUE — including the runes the window has not got room for.
    a.mouse.x = 100 + 900
    app.field_drag(&a, f, 0)
    testing.expect_value(t, d.cursors[0].head.col, 12)

    // And past the left edge it walks back the other way, to the start.
    a.mouse.x = -400
    app.field_drag(&a, f, 0)
    testing.expect_value(t, d.cursors[0].head.col, 0)

    // A drag whose capture is not ours is inert — the invariant drag.odin exists for.
    a.drag = {}
    a.mouse.x = 100 + 45
    app.field_drag(&a, f, 0)
    testing.expect_value(t, d.cursors[0].head.col, 0)

    // Grade 2 selects the word under the press and grows by whole words; grade 3 takes the whole
    // value, which is what a field's "line" is.
    a.mouse.down = true
    ...
    // column 1 is the first rune SHOWN rather than the first rune of the value.
...
// What a copy or a cut acts on. The whole-value default is the field's own: the editor's
// copy-this-line would put a trailing newline on the clipboard, and a field is a VALUE.
// (The write itself is glfw's, so it is the one step not exercised here.)
@(test)
test_field_clipboard_span :: proc(t: ^testing.T) {
    ...
    // Nothing selected: the whole value, with no newline anywhere in it.
    ...
    // A cut takes the selection out; with none it empties the value, which is the same span.
    app.doc_cut(&d)
