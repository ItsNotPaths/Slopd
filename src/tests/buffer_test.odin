...
test_buffer_join :: proc(t: ^testing.T) {
    ...
    edit.buffer_backspace(&b) // joins into the previous line
...
test_buffer_crlf_roundtrip :: proc(t: ^testing.T) {
    ...
    testing.expect(t, !edit.crlf_file("a\rb"))
    testing.expect(t, edit.crlf_file("a\r\nb\n"))
}
...
test_buffer_reload_if_changed :: proc(t: ^testing.T) {
    ...
    edit.buffer_motion(&b, .Down) // caret to line 1, past the soon-shorter end
    ...
    b.disk_mtime = {}
    testing.expect(t, !edit.buffer_reload_if_changed(&b, false))
    testing.expect(t, !b.conflict) // never raised
    testing.expect_value(t, lstr(&b, 0), "GAMMA") // edits untouched
    testing.expect(t, !edit.buffer_reload_if_changed(&b, false)) // stamp adopted -> quiet
    testing.expect(t, !b.conflict)
}
...
test_ring_contains :: proc(t: ^testing.T) {
    ...
    testing.expect(t, !edit.ring_contains(&a.editor, "/x/y.txt")) // clean
    b.dirty = true
    testing.expect(t, edit.ring_contains(&a.editor, "/x/y.txt"))
    testing.expect(t, !edit.ring_contains(&a.editor, "/x/other.txt"))
}
...
@(private = "file")
numbered :: proc(n: int) -> edit.Buffer {
...
test_scroll_follow :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, false), 0) // caret on line 0
    txt.doc_reset_cursor(&b.doc, txt.Pos{5, 0})
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, false), 0) // on screen: no move
    txt.doc_reset_cursor(&b.doc, txt.Pos{12, 0})
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, false), 3) // caret onto the bottom row
    b.scroll = 3
    txt.doc_reset_cursor(&b.doc, txt.Pos{1, 0})
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, false), 1) // above: top = caret
}
...
test_scroll_middle :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, true), 0) // clamped at the top
    txt.doc_reset_cursor(&b.doc, txt.Pos{40, 0})
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, true), 35)
    txt.doc_reset_cursor(&b.doc, txt.Pos{41, 0})
    // one line down = one row of text:
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, true), 36)
    txt.doc_reset_cursor(&b.doc, txt.Pos{99, 0})
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, true), 94) // past the end, still centred
    ...
    testing.expect_value(t, edit.buffer_scroll_target(&b, ROWS, true), 45)
}
...
test_scroll_detached_holds :: proc(t: ^testing.T) {
    ...
    for center in ([?]bool{false, true}) {
        ...
        edit.buffer_scroll_by(&b, 60, 100) // wheel: 60 lines down, detached at t=100
        ...
        edit.buffer_scroll_apply(&b, ROWS, center, 50)
        testing.expect_value(t, b.scroll, 60)
        edit.buffer_scroll_apply(&b, ROWS, center, 50)
        testing.expect_value(t, b.scroll, 60)
    }
...
test_scroll_detached_reattaches_on_input :: proc(t: ^testing.T) {
    ...
    edit.buffer_scroll_by(&b, 60, 100)
    edit.buffer_scroll_apply(&b, ROWS, false, 50) // no input since: still detached
    testing.expect_value(t, b.scroll, 60)

    edit.buffer_scroll_apply(&b, ROWS, false, 101) // a keystroke lands after the detach
    ...
    txt.doc_reset_cursor(&b.doc, txt.Pos{40, 0})
    edit.buffer_scroll_apply(&b, ROWS, false, 102)
...
test_scroll_attached_is_the_policy :: proc(t: ^testing.T) {
    ...
    edit.buffer_scroll_apply(&b, ROWS, true, 0)
    testing.expect_value(t, b.scroll, edit.buffer_scroll_target(&b, ROWS, true))
    b.scroll = 0
    edit.buffer_scroll_apply(&b, ROWS, false, 0)
    testing.expect_value(t, b.scroll, edit.buffer_scroll_target(&b, ROWS, false))
}
...
test_scroll_detached_bounds :: proc(t: ^testing.T) {
    ...
    edit.buffer_scroll_by(&b, -50, 100) // wheel up from the top
    testing.expect_value(t, b.scroll, 0)
    edit.buffer_scroll_by(&b, 5000, 100) // and far past the end
    testing.expect_value(t, b.scroll, 99)
    edit.buffer_scroll_apply(&b, ROWS, false, 50)
    testing.expect_value(t, b.scroll, 99)
...
test_buffer_save_denied :: proc(t: ^testing.T) {
    ...
    e: edit.Buffer
    defer edit.buffer_destroy(&e)
    edit.buffer_set_text(&e, "x")
    testing.expect_value(t, edit.buffer_save(&e), edit.Save_Result.No_Path)
    e.path = strings.clone("/no-such-folder-zz/x.txt")
    testing.expect_value(t, edit.buffer_save(&e), edit.Save_Result.Failed)
}
...
test_buffer_matches_disk :: proc(t: ^testing.T) {
    ...
    edit.buffer_insert_rune(&b, 'X') // edited, and the disk has not moved
    testing.expect(t, !edit.buffer_matches_disk(&b))
    ...
    u: edit.Buffer
    defer edit.buffer_destroy(&u)
    edit.buffer_set_text(&u, "x")
    testing.expect(t, !edit.buffer_matches_disk(&u))
}
...
test_hscroll_margin :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, edit.buffer_hscroll_target(0, 0, HCOLS), 0)
    testing.expect_value(t, edit.buffer_hscroll_target(0, 5, HCOLS), 0)

    // The last column that leaves the margin intact holds the view…
    testing.expect_value(t, edit.buffer_hscroll_target(0, HCOLS - 1 - P, HCOLS), 0)
    // …and one past it moves by exactly one column.
    testing.expect_value(t, edit.buffer_hscroll_target(0, HCOLS - P, HCOLS), 1)
    testing.expect_value(t, edit.buffer_hscroll_target(0, 100, HCOLS), 100 + P - HCOLS + 1)

    // Inside the padded window the view holds, which stops it sliding under every keystroke.
    testing.expect_value(t, edit.buffer_hscroll_target(89, 100, HCOLS), 89)
    testing.expect_value(t, edit.buffer_hscroll_target(89, 97, HCOLS), 89)

    // Walking back left moves the minimum too, keeping the same margin.
    testing.expect_value(t, edit.buffer_hscroll_target(89, 96, HCOLS), 88)
    testing.expect_value(t, edit.buffer_hscroll_target(89, 20, HCOLS), 12)
    // …all the way home: Home on a long line puts column 0 back on screen.
    testing.expect_value(t, edit.buffer_hscroll_target(89, 0, HCOLS), 0)
}
...
test_hscroll_narrow_pane :: proc(t: ^testing.T) {
    ...
    testing.expect_value(t, edit.buffer_hscroll_target(0, 10, 5), 10 + 2 - 5 + 1)
    testing.expect_value(t, edit.buffer_hscroll_target(8, 10, 5), 8) // and then holds
    testing.expect_value(t, edit.buffer_hscroll_target(8, 9, 5), 7)

    // One column wide: no margin is possible, so the caret sits in the only cell.
    testing.expect_value(t, edit.buffer_hscroll_target(0, 5, 1), 5)
    // A pane with no text region yet pins home rather than going negative.
    testing.expect_value(t, edit.buffer_hscroll_target(30, 40, 0), 0)
}

@(private = "file")
wide :: proc(cols: int) -> edit.Buffer {
    b := strings.builder_make(context.temp_allocator)
...
test_hscroll_apply_bounds :: proc(t: ^testing.T) {
    ...
    edit.buffer_hscroll_apply(&b, COLS, 300, 0)
    testing.expect_value(t, b.hscroll, 300 + edit.HSCROLL_PAD - COLS + 1)
    // The caret lands inside the window with the full margin to its right.
    testing.expect_value(t, 300 - b.hscroll, COLS - 1 - edit.HSCROLL_PAD)

    // A window of only short lines has nothing to scroll to, so the view pins home.
    edit.buffer_hscroll_apply(&b, COLS, 5, 0)
...
test_hscroll_detach_and_reattach :: proc(t: ^testing.T) {
    ...
    edit.buffer_hscroll_by(&b, 120, 100) // Shift+wheel right, detached at t = 100
    ...
    // dragged it back to.
    edit.buffer_hscroll_apply(&b, COLS, 300, 50)
    testing.expect_value(t, b.hscroll, 120)

    // A keystroke after the stamp re-attaches, and the policy pulls back to the caret.
    edit.buffer_hscroll_apply(&b, COLS, 300, 101)
...
test_hscroll_detached_bounds :: proc(t: ^testing.T) {
    ...
    edit.buffer_hscroll_by(&b, -50, 100) // left from home
    testing.expect_value(t, b.hscroll, 0)

    edit.buffer_hscroll_by(&b, 5000, 100) // far past the end of the longest line
    edit.buffer_hscroll_apply(&b, COLS, 300, 50)
    testing.expect_value(t, b.hscroll, 300 + edit.HSCROLL_PAD - COLS + 1)
}
...
test_hscroll_holds_while_page_detached :: proc(t: ^testing.T) {
    ...
    edit.buffer_scroll_by(&b, 60, 100)
    edit.buffer_hscroll_apply(&b, COLS, 5, 50) // only short lines drawn now
    ...
    edit.buffer_scroll_apply(&b, 10, false, 101)
    edit.buffer_hscroll_apply(&b, COLS, 300, 101)
...
test_hscroll_reset_on_set_text :: proc(t: ^testing.T) {
    b := wide(300)
    defer edit.buffer_destroy(&b)
    edit.buffer_hscroll_by(&b, 120, 100)
    edit.buffer_set_text(&b, "fresh")
    testing.expect_value(t, b.hscroll, 0)
    testing.expect_value(t, b.hscroll_detached, f64(0))
