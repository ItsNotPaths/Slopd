...
// Clay (github.com/nicbarker/clay) — a single-header immediate-mode layout library:
// declare a tree of elements, get back a flat list of render commands with resolved
// bounding boxes, plus pointer hit-testing over that same tree. Slopd uses it as the
// one source of truth for pane/row/chrome geometry, so paint and hit-test can never
// disagree. See docs/clay-refactor.md for the boundary (Clay never owns the editor
// text body or the terminal cell grid — those are Custom commands).
//
// This is UPSTREAM's official Odin binding, vendored VERBATIM except for the
// foreign import block below. It therefore keeps upstream's formatting (tabs, 180
// col) rather than Slopd's odinfmt style, deliberately: bumping Clay is then
// "re-copy the file, re-apply the one patch", with a diff that shows only real
// API churn. Unlike bindings/libvterm (first-party source, because libvterm ships
// none), nothing here is ours to maintain.
//
// PINNED: the binding and clay.h are one ABI pair and MUST be bumped together —
// CLAY_REV in download-deps.sh is the same commit this file was copied from.
// (Upstream's last tag, v0.14, predates a large clay.h rework; we track main.)

import "core:c"

// SLOPD PATCH (the only edit to this file, see the header): upstream ships a
// prebuilt clay.a per platform inside its bindings dir and imports it by relative
// path. Slopd builds the archive itself in download-deps.sh (cc + ar, like
// libvterm and the tree-sitter runtime) and is Linux-only, so the whole platform
// ladder collapses to one import reaching into gitignored vendor/.
when ODIN_OS == .Linux {
	foreign import Clay "../../vendor/clay/libclay.a"
} else {
	#panic("Slopd is Linux-only (see docs/clay-refactor.md, open decision 3)")
}
